...
router.get("/", async (req, res) => {
  try {
    ...
    if (!progress) {
      ...
      const defaultProgress = await Progress.createDefaultProgress(userId);
      return res.status(200).json({
        success: true,
        data: formatProgressForFrontend(defaultProgress, user)
      });
    }
    ...
    res.status(200).json({
      success: true,
      data: formatProgressForFrontend(progress, user)
    });
  } catch (error) {
...
router.post("/test", async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      data: {
        testId: progress.testHistory[progress.testHistory.length - 1]._id,
        accuracy,
...
// Helper functions
function formatProgressForFrontend(progress, user) {
  return {
